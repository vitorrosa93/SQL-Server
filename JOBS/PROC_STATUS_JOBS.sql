...
/*
ESTA PROCEDURE TEM COMO OBJETIVO LISTAS TOODS OS JOBS E INFORMAR O STATUS DA ULTIMA EXECUCAO
*/

CREATE OR ALTER PROC STATUS_JOBS 
AS
	BEGIN
		...
		*/
		...
		CREATE TABLE #tmp_jobhistory(   
			...
			--BLOCO 3: SELECT TABELA TEMPORARIA PARA PUXAR STATUS DOS JOBS (ULTIMA EXECUCAO)
   
			SELECT TJOBS.name_SJ AS "Nome do Job",  
			TJOBS.enabled as "Habilitado", TJOBS.QNT_Schedule_SUBQUERY as "QNT de Agendamento",
			...
			TJOBS.last_outcome_message_ended_SOS as "Mensagem",
			...
			ELSE 'Error'  
			END
			FROM #tmp_jobhistory  TJOBS WITH(NOLOCK)  
			ORDER BY 7 desc  
